    ...
    */
    ...
    class JCropThumb extends KUserDefinedField{
        ...
        function _render( $input_name, $input_id, $extra1='', $extra2=''){
            ...
            $FUNCS->load_js( JCROP_URL . 'assets/jquery.min.js' );
            $FUNCS->load_js( JCROP_URL . 'assets/jcropthumb.js' );
            ...
            if( $assoc_image ){
                ...
                if( strpos($assoc_image, $domain_prefix)===0 ){ 
					...
					$show_height = 1;
                                ...
                                $html .= '<a href="javascript:k_jcrop_thumb(\''.$input_id.'\', \''.$data.'\')" class="button"><span>Crop</span></a>';
				$html .='<br><br>';
                                $html .='<input type="hidden" id="' .$input_id. '_x" name="x" />';
                                $html .='<input type="hidden" id="' .$input_id. '_y" name="y" />';

				$html .= ( $show_width ) ? '<br><label for="w" class="jcrop">Width:</label>' : '';
                                $html .='<input ';
               		        $html .= ( $show_width ) ? 'type="text"' : 'type="hidden"';
				$html .='id="' .$input_id. '_w" name="w" readonly="readonly" />';

				$html .= ( $show_height ) ? '<br><label for="h" class="jcrop">Height:</label>' : '';
                                $html .='<input ';
               		        $html .= ( $show_height ) ? 'type="text"' : 'type="hidden"';
				$html .=' id="' .$input_id. '_h" name="h" readonly="readonly" />';
                                
                                $html .='</div><a id="' .$input_id. '_pop_button" class="recreate_tb"><span>Recreate Thumbnail</span></a>';
            ...
            if( $count==1 ){
                ...
                <script type="text/javascript">
                    function k_jcrop_thumb( field_id, data ){
                        ...
                        var x = Math.round( $(field_id + '_x').value );
                        var y = Math.round( $(field_id + '_y').value );
                        var w = Math.round( $(field_id + '_w').value );
                        var h = Math.round( $(field_id + '_h').value );
                        ...
                        var requestHTMLData = new Request(
                            {
                                ...
                                onComplete: function(response){
                                    if( response.substr(0, 7)=='OK:http' ){
                                        ...
                                        var popup = jQuery('#'+field_id+'_pop');
                                        popup.bPopup().close();
                                        ...
                                        try{
                                            $(field_id+'_tb_preview').set('src', href);
                                        }
                                        catch( e ){}
